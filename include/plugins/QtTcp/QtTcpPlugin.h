...
#ifndef __SPC_PLUGINS_QTTCP_H__
#define __SPC_PLUGINS_QTTCP_H__
...
namespace plugins {
	...
	class QtTcpSocketPlugin : public QObject, public plugins::SocketPluginInterface {
		Q_OBJECT
	...
	signals:
		void doListen(quint16);
		void doConnect(QString, quint16);
		void doSendMessage(QString);
		void doDisconnect();

	private slots:
		void newConnection();
		void receivedData();
		void onListen(quint16 port);
		void onConnect(QString ip, quint16 port);
		void onSendMessage(QString message);
		void onDisconnect();

	private:
		/**
		 * \brief incoming connections are handled via QTcpServer
		 */
		QTcpServer * _server;

		/**
		 * \brief socket for this plugin
		 */
		QTcpSocket * _socket;
		...
		/**
		 * \brief buffer storing received data to rebuild packages
		 */
