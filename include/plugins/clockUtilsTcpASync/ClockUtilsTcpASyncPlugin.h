...
#ifndef __SPC_PLUGINS_CLOCKUTILSTCPASYNC_H__
#define __SPC_PLUGINS_CLOCKUTILSTCPASYNC_H__
...
namespace plugins {
	...
	class ClockUtilsTcpSocketPlugin : public QObject, public plugins::SocketPluginInterface {
		Q_OBJECT
		Q_PLUGIN_METADATA(IID "spc.plugins.SocketPluginInterface")
		Q_INTERFACES(spc::plugins::SocketPluginInterface)

	public:
		ClockUtilsTcpSocketPlugin();

	private:
		/**
		 * \brief the sockets on the helper listening on a connection
		 * accepts just one connection
		 */
		 ...
		 */
		clockUtils::sockets::TcpSocket * _testSocket;
...
#endif /* __SPC_PLUGINS_CLOCKUTILSTCPASYNC_H__ */
